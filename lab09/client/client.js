...
const handleServerMessage = ev => {
    ...
    if (data.type === "login") {
        ...
        //     console.log("li: ", li)
        //     temp_ul.appendChild(li);
        //     console.log("ul: ", temp_ul)
        // })
        let htmlString = ""
        data.active_users.forEach(user => {
            htmlString += `<li id=${user}> ${user} </li>`
        })
        ...
        const user_to_remove = document.getElementById(data.user_left);
        console.log("user to remove: ", user_to_remove)
        ...
        if (username == data.username){
            ...
            chatMessage = `<span class="right" aria-live="polite"><b>You:</b> ${data.text}</span>`
            chatBox.innerHTML += chatMessage
        }
        else{
            let chatMessage = document.createElement("span")
            chatMessage = `<span class="left" aria-live="polite"><b>${data.username}:</b> ${data.text}</span>`
            chatBox.innerHTML += chatMessage
        }
